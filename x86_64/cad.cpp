 ...
 hoc_register_parm_default(_mechtype, &_parm_default);
  ...
  hoc_register_dparam_semantics(_mechtype, 5, "cvodeieq");
    ...
    hoc_register_var(hoc_scdoub, hoc_vdoub, hoc_intfunc);
 	ivoc_help("help ?1 cad /root/autodl-tmp/neuron-network-twcnyh/cad.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (_internalthreadargsproto_) {int _reset = 0; {
   Dcai = - ( 10000.0 ) * ( ica ) / ( 2.0 * F * depth ) - ( cai - cai0 ) / tau ;
   }
 return _reset;
}
 static int _ode_matsol1 (_internalthreadargsproto_) {
 ...
 /*END CVODE*/
 static int state (_internalthreadargsproto_) { {
    cai = cai + (1. - exp(dt*(( - ( ( 1.0 ) ) / tau ))))*(- ( ( ( - ( 10000.0 ) )*( ( ica ) ) ) / ( 2.0 * F * depth ) - ( ( ( - cai0 ) ) ) / tau ) / ( ( - ( ( 1.0 ) ) / tau ) ) - cai) ;
   }
  return 0;
}
...
static void register_nmodl_text_and_filename(int mech_type) {
    const char* nmodl_filename = "/root/autodl-tmp/neuron-network-twcnyh/cad.mod";
